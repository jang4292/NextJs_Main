...
### 코드 위치
- 도메인: `features/freecell/domain/entities`(Card, Deck, GameState, Move), `domain/value-objects`(Suit, Rank, CardColor), `domain/rules`(tableauRules, freeCellRules, foundationRules), `domain/services`(shuffle, dealInitial, moveCapacity, winCheck)
- 애플리케이션: `features/freecell/application/use-cases`(startNewGame, moveCard, undoMove)
- 프레젠테이션: `features/freecell/presentation`(FreecellGame, GameHeader, GameControls, FreeCellSlot, FoundationSlot, TableauColumn, CardFace, WinDialog) + `hooks/`(useFreecellGame, usePersistedFreecell) + `interaction/`(useFreecellInteractions, hitTest) + `styles/freecell.module.css`
- 라우트: `app/games/freecell/page.tsx`(동적 임포트, `ssr: false`) + `FreecellClient.tsx`, `app/projects/page.tsx`에 진입 링크 등록
...
- `app/games/freecell/page.tsx`의 `dynamic(..., { ssr: false })` 패턴은 2048/솔리테어/마인스위퍼와 동일하게 반복되므로 공통화 검토 여지가 있음([README.md](./README.md) 참고)
...
- **게임 허브 페이지**: `app/games/page.tsx` 부재는 네 게임 공통 이슈 — [README.md](./README.md) 참고
