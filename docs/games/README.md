...
`app/(site)/tools/games/` 하위에 있는 2048, 솔리테어(클론다이크), 마인스위퍼, 프리셀, 스도쿠 다섯 미니게임에 대한 기획/개선/개발/기능개편 문서 모음이다.
...
| 코드 경로      | `features/games/game-2048/`, `app/(site)/tools/games/2048/`   | `features/games/solitaire/`, `app/(site)/tools/games/solitaire/`                                                                  | `features/games/minesweeper/`, `app/(site)/tools/games/minesweeper/`                              | `features/games/freecell/`, `app/(site)/tools/games/freecell/`                                                    | `features/games/sudoku/`, `app/(site)/tools/games/sudoku/`                      |
...
- **게임 간 공유 코드 최소화**: 다섯 게임은 `features/games/{game}/{domain,application,presentation}` 구조를 독립적으로 갖고, catalog와 `GameHost.tsx`만 공유한다.
- **`dynamic import ssr:false` 패턴 통합**: 여러 게임의 초기 상태가 `Math.random()`에 의존해 하이드레이션 불일치가 날 수 있으므로, 클라이언트 전용 동적 임포트는 `features/games/presentation/GameHost.tsx`에서 단일 관리한다.
- **게임 허브 페이지**: `app/(site)/tools/games/page.tsx`가 게임 진입점을 한 곳에 모아 보여준다.
