...
import { undoMove, type FreecellHistory } from "../../application/use-cases/undoMove";
...
function createInitialState(): State {
  ...
  return { history: { current: game, initial: game, past: [] }, selection: null };
}
...
function applyMove(state: State, source: CardLocation, destination: CardLocation): State {
  const game = state.history.current;
  const cardIds = (getMovingCards(game, source) ?? []).map((movingCard) => movingCard.id);
  const result = moveCard(game, { source, destination, cardIds });

  if (!result.success) return { ...state, selection: null };

  return {
    history: { ...state.history, current: result.nextState, past: [...state.history.past, game] },
    selection: null,
  };
}
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "RESTART": {
      ...
      return { history: { current: initial, initial, past: [] }, selection: null };
    }
    ...
    case "SELECT_OR_MOVE": {
      ...
      if (!state.selection) {
        return hasMovableCard(game, location) ? { ...state, selection: location } : state;
      }
      ...
      return hasMovableCard(game, location) ? { ...state, selection: location } : { ...state, selection: null };
    }
...
export function useFreecellGame() {
  ...
  return {
    ...
    selectOrMove: (location: CardLocation) => dispatch({ type: "SELECT_OR_MOVE", location }),
    commitMove: (source: CardLocation, destination: CardLocation) =>
      dispatch({ type: "COMMIT_MOVE", source, destination }),
    hydrate: (history: FreecellHistory) => dispatch({ type: "HYDRATE", history }),
  };
}
