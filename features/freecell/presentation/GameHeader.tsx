...
export function GameHeader({ elapsedSeconds, moveCount }: GameHeaderProps) {
  return (
    <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-2 sm:px-4">
      ...
      <div className="flex items-center gap-4 text-sm text-neutral-600" role="status">
        <span aria-label={`경과 시간 ${formatElapsed(elapsedSeconds)}`}>⏱ {formatElapsed(elapsedSeconds)}</span>
