...
import type { CardLocation, MoveCommand, MoveResult } from "../../domain/entities/Move";
import { isSameLocation } from "../../domain/entities/Move";
import { canPlaceOnTableau, isValidSequence } from "../../domain/rules/tableauRules";
...
export function getMovingCards(state: GameState, source: CardLocation): Card[] | null {
...
function removeFromSource(state: GameState, source: CardLocation, count: number): GameState {
  if (source.type === "freeCell") {
    const freeCells = state.freeCells.map((slot, index) => (index === source.slotIndex ? null : slot));
...
export function moveCard(state: GameState, command: MoveCommand): MoveResult {
  ...
  if (destination.type === "foundation") {
    if (movingCards.length !== 1) return { success: false, reason: "INVALID_DESTINATION" };
    if (leadCard.suit !== destination.suit) return { success: false, reason: "INVALID_DESTINATION" };
    ...
    const foundations = {
      ...
      [destination.suit]: [...afterRemoval.foundations[destination.suit], leadCard],
    };
    return { success: true, nextState: finalizeMove({ ...afterRemoval, foundations }) };
  }
  ...
  if (destination.type === "freeCell") {
    ...
    return { success: true, nextState: finalizeMove({ ...afterRemoval, freeCells }) };
  }
  ...
  return { success: true, nextState: finalizeMove({ ...afterRemoval, tableau }) };
}
