...
describe("getMovingCards", () => {
  it("returns null for an empty free cell", () => {
    ...
    expect(getMovingCards(state, { type: "freeCell", slotIndex: 0 })).toBeNull();
  });

  it("returns the single card held in an occupied free cell", () => {
    const state = buildGameState({ freeCells: [card("spades", 5), null, null, null] });
    expect(getMovingCards(state, { type: "freeCell", slotIndex: 0 })).toEqual([card("spades", 5)]);
  });

  it("returns a valid alternating-color run from a tableau column", () => {
    const pile = [card("clubs", 10), card("spades", 9), card("hearts", 8), card("clubs", 7)];
    const state = buildGameState({ tableau: [pile, [], [], [], [], [], [], []] });
    const moving = getMovingCards(state, { type: "tableau", columnIndex: 0, cardIndex: 1 });
    expect(moving?.map((c) => c.id)).toEqual([card("spades", 9).id, card("hearts", 8).id, card("clubs", 7).id]);
  });

  it("returns null when the tableau suffix is not a valid sequence", () => {
    const pile = [card("clubs", 10), card("spades", 9), card("clubs", 8)];
    const state = buildGameState({ tableau: [pile, [], [], [], [], [], [], []] });
    expect(getMovingCards(state, { type: "tableau", columnIndex: 0, cardIndex: 0 })).toBeNull();
  });

  it("returns null for a foundation source", () => {
    const state = buildGameState({ foundations: { spades: [card("spades", 1)], hearts: [], diamonds: [], clubs: [] } });
    expect(getMovingCards(state, { type: "foundation", suit: "spades" })).toBeNull();
  });
});

describe("moveCard - free cell moves", () => {
  it("moves a tableau top card into an empty free cell", () => {
    const state = buildGameState({ tableau: [[card("hearts", 5)], [], [], [], [], [], [], []] });
...
describe("moveCard - foundation moves", () => {
  it("moves an Ace onto an empty foundation", () => {
    const state = buildGameState({ tableau: [[card("hearts", 1)], [], [], [], [], [], [], []] });
    const result = moveCard(state, {
      source: { type: "tableau", columnIndex: 0, cardIndex: 0 },
  ...
  it("rejects a non-Ace onto an empty foundation", () => {
    ...
    expect(result).toEqual({ success: false, reason: "FOUNDATION_ORDER_MISMATCH" });
  });

  it("rejects placing a card from a free cell onto the wrong suit's foundation", () => {
    const state = buildGameState({
      freeCells: [card("spades", 2), null, null, null],
      foundations: { spades: [], hearts: [card("hearts", 1)], diamonds: [], clubs: [] },
    });
    const result = moveCard(state, {
...
describe("moveCard - tableau moves", () => {
  it("moves a valid alternating-color run onto a matching target", () => {
    ...
    const state = buildGameState({ tableau: [source, target, [], [], [], [], [], []] });
  ...
  it("rejects a same-color destination", () => {
    const state = buildGameState({
      tableau: [[card("spades", 7)], [card("clubs", 8)], [], [], [], [], [], []],
    });
    const result = moveCard(state, {
      source: { type: "tableau", columnIndex: 0, cardIndex: 0 },
      destination: { type: "tableau", columnIndex: 1, cardIndex: 0 },
  ...
  it("allows any card onto an empty column", () => {
    const state = buildGameState({ tableau: [[card("spades", 7)], [], [], [], [], [], [], []] });
    const result = moveCard(state, {
      source: { type: "tableau", columnIndex: 0, cardIndex: 0 },
      destination: { type: "tableau", columnIndex: 1, cardIndex: 0 },
      cardIds: [card("spades", 7).id],
  ...
  it("rejects a supermove that exceeds the calculated capacity", () => {
    ...
    const state = buildGameState({
      ...
      freeCells: [card("diamonds", 2), card("diamonds", 3), card("diamonds", 4), card("diamonds", 5)],
    });
    const result = moveCard(state, {
      source: { type: "tableau", columnIndex: 0, cardIndex: 0 },
      destination: { type: "tableau", columnIndex: 1, cardIndex: 0 },
    ...
    expect(result).toEqual({ success: false, reason: "MOVE_CAPACITY_EXCEEDED" });
  });
