...
describe("dealInitial", () => {
  it("deals all 52 cards into 8 tableau columns", () => {
    ...
    const totalDealt = state.tableau.reduce((sum, pile) => sum + pile.length, 0);
    expect(totalDealt).toBe(52);
  });

  it("deals 7 cards to the first four columns and 6 to the rest", () => {
    const state = dealInitial(createDeck());
    expect(state.tableau.slice(0, 4).map((pile) => pile.length)).toEqual([7, 7, 7, 7]);
    expect(state.tableau.slice(4).map((pile) => pile.length)).toEqual([6, 6, 6, 6]);
  });
  ...
  it("starts with empty free cells and foundations", () => {
    ...
    expect(Object.values(state.foundations).every((pile) => pile.length === 0)).toBe(true);
  });
