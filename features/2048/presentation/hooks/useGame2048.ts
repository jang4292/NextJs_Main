...
import { reconcileTiles, tilesFromBoard, type Tile } from "../../application/use-cases/reconcileTiles";
...
function reducer(state: UIState, action: Action): UIState {
  switch (action.type) {
    case "MOVE": {
      ...
      const tiles = reconcileTiles(state.tiles, result.traces, result.spawned, action.nextId);
    ...
    case "CLEAR_TILE_FLAGS": {
      if (state.tiles.every((tile) => !tile.isNew && !tile.isMerged)) return state;
      return {
        ...state,
...
export function useGame2048() {
  ...
  return {
    ...
    move: (direction: Direction) => dispatch({ type: "MOVE", direction, nextId: idGen }),
