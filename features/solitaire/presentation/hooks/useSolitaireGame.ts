...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "DRAW_OR_RECYCLE": {
      const game =
        state.game.stock.length > 0 ? drawFromStock(state.game) : recycleWaste(state.game);
    ...
    case "CLICK_TABLEAU": {
      ...
      if (state.selection) {
        const result = moveCard(state.game, state.selection, { zone: "tableau", column });
    ...
    case "CLICK_WASTE": {
      ...
      return topCard ? { ...state, selection: { zone: "waste" } } : { ...state, selection: null };
    }
