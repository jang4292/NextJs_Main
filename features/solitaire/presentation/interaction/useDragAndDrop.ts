 ...
 */
export function useDragAndDrop({ game, isLocked, onCommit, withLock }: UseDragAndDropOptions) {
    ...
    (source: MoveSource) => ({
      ...
      onPointerUp(event: ReactPointerEvent<HTMLElement>) {
        ...
        void withLock(async () => {
          ...
          if (!moved) {
            await flyElement(ghostEl, dx, dy, { durationMs: RETURN_DURATION_MS }).finished.catch(() => {});
            cleanup(pending.originEl);
            return;
          }
          ...
          if (finalRect) {
            ...
            await flyElement(ghostEl, correctionX, correctionY, { durationMs: SNAP_DURATION_MS }).finished.catch(
              () => {},
            );
          }
          cleanup(pending.originEl);
  ...
  return { dragVisual, hiddenCardIds, ghostRef, bindDragHandle, consumeSuppressedClick };
}
