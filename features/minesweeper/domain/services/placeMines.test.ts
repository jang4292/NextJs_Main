...
describe("placeMines", () => {
  it("places exactly mineCount mines", () => {
    ...
    const result = placeMines(board, 10, [{ row: 4, column: 4 }], fixedRng(Array(10).fill(0)));
  ...
  it("never places two mines on the same cell", () => {
    const board = createEmptyBoard(9, 9);
    const result = placeMines(board, 10, [{ row: 0, column: 0 }], fixedRng(Array(10).fill(0)));
  ...
  it("never places a mine on an excluded (safe) position", () => {
    ...
    const result = placeMines(board, 8, [safePosition], fixedRng(Array(8).fill(0)));
  ...
  it("places mines deterministically at the positions selected by an injected RNG", () => {
    const board = createEmptyBoard(3, 3);
    const result = placeMines(board, 3, [{ row: 1, column: 1 }], fixedRng([0, 0, 0]));
