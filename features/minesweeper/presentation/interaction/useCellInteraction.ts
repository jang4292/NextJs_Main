 ...
 */
export function useCellInteraction({ flagMode, onReveal, onToggleFlag }: UseCellInteractionOptions) {
  ...
  const onContextMenu = useCallback(
    (event: ReactMouseEvent<HTMLElement>, position: Position) => {
      ...
      const isLongPressEcho = Date.now() - lastLongPressAtRef.current < CONTEXT_MENU_ECHO_WINDOW_MS;
