...
type Action = { type: "REVEAL"; position: Position } | { type: "TOGGLE_FLAG"; position: Position } | { type: "RESTART" };
...
export function useMinesweeper(difficulty: Difficulty = BEGINNER) {
  ...
  const { elapsedSeconds, reset: resetTimer } = useElapsedTimer(state.status === "playing");
  ...
  return {
    ...
    toggleFlag: (position: Position) => dispatch({ type: "TOGGLE_FLAG", position }),
    restart: () => {
