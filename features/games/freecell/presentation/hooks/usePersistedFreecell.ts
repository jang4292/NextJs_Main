...
function isGameState(value: unknown): value is GameState {
  ...
  return (
    ...
    Array.isArray(candidate.freeCells) &&
...
function readSavedGame(): SavedFreecellGame | null {
  try {
    ...
    if (!isGameState(parsed.currentState) || !isGameState(parsed.initialState))
      return null;
...
export function usePersistedFreecell() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      freecell.hydrate({
        current: saved.currentState,
        initial: saved.initialState,
        past: saved.history,
      });
    }
