 ...
 */
export function useFreecellInteractions({
  game,
  isLocked,
  onCommit,
}: UseFreecellInteractionsOptions) {
  const pendingRef = useRef<PendingDrag | null>(null);
  const suppressClickRef = useRef(false);

  const resetCardEls = useCallback(
    (cardEls: HTMLElement[], animateReturn: boolean) => {
    ...
    (source) => ({
      onPointerDown(event) {
        ...
        if (source.type === "tableau") {
          const columnEl = originEl.closest<HTMLElement>(
            '[data-drop-zone="tableau"]',
          );
          if (columnEl) {
            cardEls = Array.from(
              columnEl.querySelectorAll<HTMLElement>("[data-card-index]"),
            ).filter((el) => Number(el.dataset.cardIndex) >= source.cardIndex);
          }
        }
