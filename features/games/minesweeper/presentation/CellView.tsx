...
function getCellAriaLabel(cell: Cell): string {
  if (cell.isRevealed) {
    return cell.isMine
      ? "지뢰 셀"
      : `주변 지뢰 ${cell.adjacentMines}개가 있는 열린 셀`;
  }
...
export function CellView({ cell, interaction }: CellViewProps) {
  ...
  return (
    <button
      type="button"
      className={cn(
        "aspect-square touch-none rounded-sm text-sm font-bold select-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 focus-visible:outline-none",
        styles.cell,
      )}
